...
def create_case(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            print("status:",status)
            case.save()
            time = Case.objects.values_list("created_on",flat=True).filter(name=name)
            print(".....",time)
            # for t in time:
            #     dtime = t.created_on
            #     dt = dtime.strftime("%H:%M:%S")
            #     print("time:",dt)
            #     # a=tt
            #     time.append(dt)
...
def ipecs(request):
    if request.method == 'POST':
        ...
        print("enter)(()_())")   
        if (Case.objects.filter(staffname=s).exists()):
            visitor = Case.objects.get(staffname=s)
            id = visitor.id
