...
class HomeView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        d = date.today()
        print("DATE:",d)
        open = Case.objects.filter(Q(status="Open")& Q(created_on__icontains=d)).count()
        inprogress = Case.objects.filter(Q(status="In Progress")& Q(created_on__icontains=d)).count()
        closed = Case.objects.filter(Q(status="Close")& Q(created_on__icontains=d)).count()
        ...
        contact = Contact.objects.filter(created_on__icontains=d).count()
        print("CCOONNTTAACCTT:",contact)
        case = Case.objects.filter(created_on__icontains=d).count()
        print("case:",case)
        account = Account.objects.filter(created_on__icontains=d).count()
