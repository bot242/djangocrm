        ...
        if d == None and tod == None:
            ...
            if self.request.user.role == "ADMIN" or self.request.user.is_superuser:  
                ...
                for i in idd:
                    ...
                    user = i['username']
                    # print("USER:",user)
                    i_open = Case.objects.filter(Q(status="Open")& Q(created_by=id)& Q(created_on__range=[fd,td])).count()
                    ...
                    # print("oc:",i_open)
                    i_inprogress = Case.objects.filter(Q(status="In Progress")& Q(created_by=id)& Q(created_on__range=[fd,td])).count()
                    # print()
                    i_closed = Case.objects.filter(Q(status="Closed")& Q(created_by=id)& Q(created_on__range=[fd,td])).count()
                    i_totalcases = Case.objects.filter(Q(created_by=id)& Q(created_on__range=[fd,td])).count()
                    temp['username'] = user
                    temp['open'] = i_open
                    temp['progress']=i_inprogress
                    temp['closed'] = i_closed
...
class HomeView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    #     return render(request, 'sales/sales_dash.html', {'output' : angulargaugeWidget.render()})
    ...
    def get_context_data(self, **kwargs):
            ...
            if self.request.user.role == "ADMIN" or self.request.user.is_superuser:  
                ...
                for dt in rrule(DAILY, dtstart=a, until=b):
                    ...
                    if td == bn:
                        ...
                        op_qualify = Opportunity.objects.filter(Q(stage="QUALIFICATION")& Q(update_date__icontains=bn)).count()
                        qualification_dts.append(op_qualify)

                        op_analysis = Opportunity.objects.filter(Q(stage="NEEDS ANALYSIS")& Q(update_date__icontains=bn)).count()
                        needanalysis_dts.append(op_analysis)

                        op_value = Opportunity.objects.filter(Q(stage="VALUE PROPOSITION")& Q(update_date__icontains=bn)).count()
                        valueproposition_dts.append(op_value)

                        op_decision = Opportunity.objects.filter(Q(stage="ID.DECISION MAKERS")& Q(update_date__icontains=bn)).count()
                        decisionmakers_dts.append(op_decision)
                        
                        op_perception = Opportunity.objects.filter(Q(stage="PERCEPTION ANALYSIS")& Q(update_date__icontains=bn)).count()
                        perception_dts.append(op_perception)

                        op_quote = Opportunity.objects.filter(Q(stage="PROPOSAL/PRICE QUOTE")& Q(update_date__icontains=bn)).count()
                        proposalquote_dts.append(op_quote)

                        op_review = Opportunity.objects.filter(Q(stage="NEGOTIATION/REVIEW")& Q(update_date__icontains=bn)).count()
                        negotiation_dts.append(op_review)

                        op_won = Opportunity.objects.filter(Q(stage="CLOSED WON")& Q(update_date__icontains=bn)).count()
                        closedwon_dts.append(op_won)
        ...
        ##################################### Column Graph values for the sum of amount  ###########################################
                qualification = Opportunity.objects.filter(Q(stage="QUALIFICATION")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if qualification['total_amount']==None:
                    qualification_amount=0
                else:
                    qualification_amount=qualification['total_amount']

                needanalysis = Opportunity.objects.filter(Q(stage="NEEDS ANALYSIS")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if needanalysis['total_amount']==None:
                    needanalysis_amount=0
                else:
                    needanalysis_amount=needanalysis['total_amount']

                valueproposition = Opportunity.objects.filter(Q(stage="VALUE PROPOSITION")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if valueproposition['total_amount']==None:
                    valueproposition_amount=0
                else:
                    valueproposition_amount=valueproposition['total_amount']
                # print(valueproposition_amount,"vpa")
                decisionmakers = Opportunity.objects.filter(Q(stage="ID.DECISION MAKERS")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if decisionmakers['total_amount']==None:
                    decisionmakers_amount=0
                else:
                    decisionmakers_amount=decisionmakers['total_amount']

                perception = Opportunity.objects.filter(Q(stage="PERCEPTION ANALYSIS")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if perception['total_amount']==None:
                    perception_amount=0
                else:
                    perception_amount=perception['total_amount']
                
                proposalquote = Opportunity.objects.filter(Q(stage="PROPOSAL/PRICE QUOTE")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if proposalquote['total_amount']==None:
                    proposalquote_amount=0
                else:
                    proposalquote_amount=proposalquote['total_amount']

                negotiation = Opportunity.objects.filter(Q(stage="NEGOTIATION/REVIEW")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if negotiation['total_amount']==None:
                    negotiation_amount=0
                else:
                    negotiation_amount=negotiation['total_amount']

                closedwon = Opportunity.objects.filter(Q(stage="CLOSED LOST")& Q(created_on__range=[fd,td])).aggregate(total_amount=Sum('amount'))
                if closedwon['total_amount']==None:
                    closedwon_amount=0
                else:
                    closedwon_amount=closedwon['total_amount']
                ...
                # print("Need Analysis",funnel_needanalysis)
                funnel_value_proposition = vp+im+pa+pq+nr+cw+cl
                # print("Value Proposition",funnel_value_proposition)
                funnel_decision_makers = im+pa+pq+nr+cw+cl
                # print("Decision makers",funnel_decision_makers)
                funnel_perception_analysis = pa+pq+nr+cw+cl
                # print("Perception analysis",funnel_perception_analysis)
        ...
        ################################ Bar Graph values for Lead by Source #########################################################
                ...
                context['qualification_dts'] = qualification_dts
                context['needanalysis_dts'] = needanalysis_dts
                context['valueproposition_dts'] = valueproposition_dts
                context['decisionmakers_dts'] = decisionmakers_dts
                context['perception_dts'] = perception_dts
                context['proposalquote_dts'] = proposalquote_dts
                context['negotiation_dts'] = negotiation_dts
                context['closedwon_dts'] = closedwon_dts
                ...
                for dt in rrule(DAILY, dtstart=a, until=b):
                    ...
                    if td == bn:
                        ...
                        op_qualify = Opportunity.objects.filter(Q(stage="QUALIFICATION",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="QUALIFICATION",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        qualification_dts.append(op_qualify)

                        op_analysis = Opportunity.objects.filter(Q(stage="NEEDS ANALYSIS",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="NEED ANALYSIS",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        needanalysis_dts.append(op_analysis)

                        op_value = Opportunity.objects.filter(Q(stage="VALUE PROPOSITION",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="VALUE PROPOSITION",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        valueproposition_dts.append(op_value)

                        op_decision = Opportunity.objects.filter(Q(stage="ID.DECISION MAKERS",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="ID.DECISION MAKERS",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        decisionmakers_dts.append(op_decision)
                        
                        op_perception = Opportunity.objects.filter(Q(stage="PERCEPTION ANALYSIS",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="PERCEPTION ANALYSIS",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        perception_dts.append(op_perception)

                        op_quote = Opportunity.objects.filter(Q(stage="PROPOSAL/PRICE QUOTE",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="PROPOSAL/PRICE QUOTE",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        proposalquote_dts.append(op_quote)

                        op_review = Opportunity.objects.filter(Q(stage="NEGOTIATION/REVIEW",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="NEGOTIATION/REVIEW",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        negotiation_dts.append(op_review)

                        op_won = Opportunity.objects.filter(Q(stage="CLOSED WON",update_date__icontains=bn,created_by_id=self.request.user.id)|Q(stage="CLOSED WON",update_date__icontains=bn,assigned_to=self.request.user.id)).count()
                        closedwon_dts.append(op_won)
        ...
        ##################################### Column Graph values for the sum of amount  ###########################################
                qualification = Opportunity.objects.filter(Q(stage="QUALIFICATION",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="QUALIFICATION",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if qualification['total_amount']==None:
                    qualification_amount=0
                else:
                    qualification_amount=qualification['total_amount']

                needanalysis = Opportunity.objects.filter(Q(stage="NEEDS ANALYSIS",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="NEEDS ANALYSIS",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if needanalysis['total_amount']==None:
                    needanalysis_amount=0
                else:
                    needanalysis_amount=needanalysis['total_amount']

                valueproposition = Opportunity.objects.filter(Q(stage="VALUE PROPOSITION",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="VALUE PROPOSITION",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if valueproposition['total_amount']==None:
                    valueproposition_amount=0
                else:
                    valueproposition_amount=valueproposition['total_amount']
                # print(valueproposition_amount,"vpa")
                decisionmakers = Opportunity.objects.filter(Q(stage="ID.DECISION MAKERS",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="ID.DECISION MAKERS",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if decisionmakers['total_amount']==None:
                    decisionmakers_amount=0
                else:
                    decisionmakers_amount=decisionmakers['total_amount']

                perception = Opportunity.objects.filter(Q(stage="PERCEPTION ANALYSIS",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="PERCEPTION ANALYSIS",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if perception['total_amount']==None:
                    perception_amount=0
                else:
                    perception_amount=perception['total_amount']
                
                proposalquote = Opportunity.objects.filter(Q(stage="PROPOSAL/PRICE QUOTE",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="PROPOSAL/PRICE QUOTE",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if proposalquote['total_amount']==None:
                    proposalquote_amount=0
                else:
                    proposalquote_amount=proposalquote['total_amount']

                negotiation = Opportunity.objects.filter(Q(stage="NEGOTIATION/REVIEW",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="NEGOTIATION/REVIEW",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if negotiation['total_amount']==None:
                    negotiation_amount=0
                else:
                    negotiation_amount=negotiation['total_amount']

                closedwon = Opportunity.objects.filter(Q(stage="CLOSED LOST",created_on__range=[fd,td],created_by_id=self.request.user.id)|Q(stage="CLOSED LOST",created_on__range=[fd,td],assigned_to=self.request.user.id)).aggregate(total_amount=Sum('amount'))
                if closedwon['total_amount']==None:
                    closedwon_amount=0
                else:
                    closedwon_amount=closedwon['total_amount']
        ...
        ##################################### Column Graph values for the sum of amount  ###########################################
                ...
                x = [funnel_qualification,funnel_needanalysis,funnel_value_proposition,funnel_decision_makers,funnel_perception_analysis,funnel_proposal,funnel_negotiation,funnel_closed_won],
                text = [qualification_amount,needanalysis_amount,valueproposition_amount,decisionmakers_amount,perception_amount,proposalquote_amount,negotiation_amount,closedwon_amount],
                marker = {"color": ["rgb(92, 185, 228)", "rgb(81, 153, 235)", "rgb(36, 115, 206)", "rgb(26, 90, 163)", "rgb(13, 62, 221)", "rgb(6, 48, 187)", "rgb(4, 30, 114)", "rgb(7, 23, 77)"]},
                hoverinfo = 'text')
