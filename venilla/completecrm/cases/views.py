...
class CasesListInd(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    model = Case
    context_object_name = "cases"
    ...
    def get_queryset(self):
        bnmm = self.request.user.username
        name_id = User.objects.values_list('id',flat=True).get(username=bnmm)
        queryset = self.model.objects.all().select_related("account").order_by('-id').filter(Q(assigned_to=name_id)|Q(created_by=name_id))
    ...
    def get_context_data(self, **kwargs):
        ...
        context["data"] = Case.objects.all().filter(Q(assigned_to=name_id)|Q(created_by=name_id)).order_by('-id')
        context["accounts"] = Account.objects.filter(status="open")
        context["per_page"] = self.request.POST.get('per_page')
