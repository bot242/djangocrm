...
def service_access_required(function):
    """ this function is a decorator used to authorize if a user has marketing access """
    def wrap(request, *args, **kwargs):
        if request.user.role == 'ADMIN' or request.user.is_superuser or request.user.has_service_access:
            return function(request, *args, **kwargs)
        else:
            raise PermissionDenied
    return wrap
...
class ServiceAccessRequiredMixin(AccessMixin):
    """ Mixin used to authorize if a user has service access  """
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.raise_exception = True
        if request.user.role == 'ADMIN' or request.user.is_superuser or request.user.has_service_access:
            return super(ServiceAccessRequiredMixin, self).dispatch(request, *args, **kwargs)
        else:
            return self.handle_no_permission()
