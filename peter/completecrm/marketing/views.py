...
def dashboard(request):
    ...
    d = request.GET.get('status')
    print("Start date:",d)

    tod = request.GET.get('tostatus')
    print('End date:',tod)
    ...
    if d == None and tod == None:
        ...
        if request.user.role == 'ADMIN' or request.user.is_superuser:
            ...
            for dt in rrule(DAILY, dtstart=a, until=b):
                ...
                if td == bn:
                    ...
                    m_scheduled = Campaign.objects.filter(status="Scheduled",created_on__icontains=bn).count()
                    scheduled_data.append(m_scheduled)

                    m_preparing = Campaign.objects.filter(status="Preparing",created_on__icontains=bn).count()
                    preparing_data.append(m_preparing)

                    m_cancelled = Campaign.objects.filter(status="Cancelled",created_on__icontains=bn).count()
                    cancelled_data.append(m_cancelled)

                    m_sent = Campaign.objects.filter(status="Sent",created_on__icontains=bn).count()
                    sent_data.append(m_sent)

                    m_sending = Campaign.objects.filter(status="Sending",created_on__icontains=bn).count()
                    sending_data.append(m_sending)

            
            email_templates = EmailTemplate.objects.filter(created_on__range=[fd,td]).count()
            print(email_templates, "email template")
            contacts = Contact.objects.filter(created_on__range=[fd,td]).count()
            print(contacts, "contacts")
            campaign = Campaign.objects.all().filter(created_on__range=[fd,td])
            print(campaign,"campaigns")
            contacts_list = ContactList.objects.filter(created_on__range=[fd,td])

            leads = Lead.objects.filter(created_on__range=[fd,td]).count()
            print(leads,"--------------------------------------------")

            scheduled = Campaign.objects.values_list('status',flat=True).filter(status="Scheduled",created_on__range=[fd,td]).count()
            cancelled = Campaign.objects.values_list('status',flat=True).filter(status="Cancelled",created_on__range=[fd,td]).count()
            sending = Campaign.objects.values_list('status',flat=True).filter(status="Sending",created_on__range=[fd,td]).count()
            preparing = Campaign.objects.values_list('status',flat=True).filter(status="Preparing",created_on__range=[fd,td]).count()
            sent = Campaign.objects.values_list('status',flat=True).filter(status="Sent",created_on__range=[fd,td]).count()
            ...
            for i in idd:
                ...
                new = Lead.objects.filter(status="new",created_on__range=[fd,td],created_by_id=id).count()
                print("NEW:",new)
                working = Lead.objects.filter(status="working",created_on__range=[fd,td],created_by_id=id).count()
                print("Working:",working)
                qualified = Lead.objects.filter(status="qualified",created_on__range=[fd,td],created_by_id=id).count()
                print("Qualified:",qualified)
                unqualified = Lead.objects.filter(status="unqualified",created_on__range=[fd,td],created_by_id=id).count()
                print("UnQualified:",unqualified)
                replaced = Lead.objects.filter(status="replaced",created_on__range=[fd,td],created_by_id=id).count()
                print("Replaced:",replaced)
                closed = Lead.objects.filter(status="closed",created_on__range=[fd,td],created_by_id=id).count()
                print("Closed:",closed)
                temp['username']=user
                temp['new']=new
                temp['working']=working
                temp['qualified']=qualified
                temp['unqualified']=unqualified
                temp['replaced']=replaced
                temp['closed']=closed
                data.append(temp)

            print(data,"status_data_for_lead")

            totalreason = Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td]).count()
            print(totalreason,"llllll")
            if totalreason == 0:
                nb_percentage=0
                comp_percentage=0
                nd_percentage=0
                ...
                nb= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="No Budget").count()
                nb_percentage = nb * 100/totalreason
                print("NO budget",nb_percentage)

                comp= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Competitor").count()
                comp_percentage = comp * 100/totalreason
                print("Competitor",comp_percentage)

                nd= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="No Decision").count()
                nd_percentage = nd * 100/totalreason
                print("NO Decision",nd_percentage)

                np= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="No Power").count()
                np_percentage = np * 100/totalreason
                print("NO Power",np_percentage)

                lv= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Lack of Vision").count()
                lv_percentage = lv * 100/totalreason
                print("Lack of Vision",lv_percentage)

                nr= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Never Reached").count()
                nr_percentage = nr * 100/totalreason
                print("Never Reached",nr_percentage)

                wi= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Wrong Information").count()
                wi_percentage = wi * 100/totalreason
                print("Wrong Information",wi_percentage)
            ...
            print('no')
            email_templates = EmailTemplate.objects.filter(created_by=request.user,created_on__range=[fd,td]).count()
            print(email_templates, 'email template')
            # email_templates = EmailTemplate.objects.filter().count()
            contacts = Contact.objects.filter(created_by=request.user,created_on__range=[fd,td])
            campaign = Campaign.objects.filter(created_by=request.user,created_on__range=[fd,td])
            contacts_list = ContactList.objects.filter(created_by=request.user,created_on__range=[fd,td])
            leads = Lead.objects.filter(created_on__range=[fd,td]).count()

            scheduled = Campaign.objects.values_list('status',flat=True).filter(status="Scheduled",created_by=request.user,created_on__range=[fd,td]).count()
            cancelled = Campaign.objects.values_list('status',flat=True).filter(status="Cancelled",created_by=request.user,created_on__range=[fd,td]).count()
            sending = Campaign.objects.values_list('status',flat=True).filter(status="Sending",created_by=request.user,created_on__range=[fd,td]).count()
            preparing = Campaign.objects.values_list('status',flat=True).filter(status="Preparing",created_by=request.user,created_on__range=[fd,td]).count()
            sent = Campaign.objects.values_list('status',flat=True).filter(status="Sent",created_by=request.user,created_on__range=[fd,td]).count()
            
            # new = Lead.objects.filter(status="new",created_on__range=[fd,td],created_by=request.user).count()
            # print("NEW:",new)
            # working = Lead.objects.filter(status="working",created_on__range=[fd,td],created_by=request.user).count()
            # print("Working:",working)
            # qualified = Lead.objects.filter(status="qualified",created_on__range=[fd,td],created_by=request.user).count()
            # print("Qualified:",qualified)
            # unqualified = Lead.objects.filter(status="unqualified",created_on__range=[fd,td],created_by=request.user).count()
            # print("UnQualified:",unqualified)
            # replaced = Lead.objects.filter(status="replaced",created_on__range=[fd,td],created_by=request.user).count()
            # print("Replaced:",replaced)
            # closed = Lead.objects.filter(status="closed",created_on__range=[fd,td],created_by=request.user).count()
            # print("Closed:",closed)

            totalreason = Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],created_by=request.user).count()
            print(totalreason,"llllll")
            if totalreason == 0:
                nb_percentage=0
                comp_percentage=0
                ...
                nb= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="No Budget",created_by=request.user).count()
                nb_percentage = nb * 100/totalreason
                print("NO budget",nb_percentage)

                comp= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Competitor",created_by=request.user).count()
                comp_percentage = comp * 100/totalreason
                print("Competitor",comp_percentage)

                nd= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="No Decision",created_by=request.user).count()
                nd_percentage = nd * 100/totalreason
                print("NO Decision",nd_percentage)

                np= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="No Power",created_by=request.user).count()
                np_percentage = np * 100/totalreason
                print("NO Power",np_percentage)

                lv= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Lack of Vision",created_by=request.user).count()
                lv_percentage = lv * 100/totalreason
                print("Lack of Vision",lv_percentage)

                nr= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Never Reached",created_by=request.user).count()
                nr_percentage = nr * 100/totalreason
                print("Never Reached",nr_percentage)

                wi= Lead.objects.all().filter(status='unqualified',created_on__range=[fd,td],reason_for_unqualified_lead="Wrong Information",created_by=request.user).count()
                wi_percentage = wi * 100/totalreason
                print("Wrong Information",wi_percentage)
        ...
        context = {
            ...
            'data':d,
            'tod':tod,
            'case_open':date_field,
            'leads':leads,
            'nb':nb_percentage,
            'comp':comp_percentage,
            'nd':nd_percentage,
            'np':np_percentage,
            'lv':lv_percentage,
            'nr':nr_percentage,
...
def contacts_list(request):
    if (request.user.role == "ADMIN"):
        ...
        contact_ids = request.user.marketing_contactlist.all().values_list('contacts', flat=True)
...
def delete_contact(request, pk):
    ...
    if request.GET.get('from_contact'):
        if contact_obj.contact_list.count() == 1:
            ...
            contact_obj.contact_list.remove(contact_list_obj)
            # update_elastic_search_index.delay()
            return redirect(reverse('marketing:contact_list_detaimarketing/lists/detl', args=(request.GET.get('from_contact'),)))
    else:
        contact_obj.delete()
        # update_elastic_search_index.delay()
...
#########################     details.html datas              ##########################
    ...
    data = {'campaigns_list': queryset, 'users': users, 'contact_lists': contact_lists}
    # "campaign": campaign, "contacts": contacts, 'unsubscribe_contacts': unsubscribe_contacts,
...
def campaign_details(request, pk):
    ...
    return render(request, 'marketing/campaign/details.html', {'contact_lists': contact_lists, "campaign": campaign,
...
def contacts_list_elastic_search(request):
    if (request.user.role == "ADMIN"):
        ...
        contact_lists = ContactList.objects.all()
        # print(contact_lists,"list of contacts")
        con_tacts = Contact.objects.all()
        # print(con_tacts,"testing of contacts")
    ...
    if request.method == 'GET':
        context = {'contacts': contacts, 'users': users, 'con_tacts':con_tacts, 'contact_lists': contact_lists,
    ...
    if request.method == 'POST':
        data = request.POST
        if data.get('name'):
            contacts = contacts.filter(name__icontains=data.get('name'))
            bounced_contacts = bounced_contacts.filter(name__icontains=data.get('name'))
            failed_contacts = failed_contacts.filter(name__icontains=data.get('name'))
        if data.get('email'):
        ...
        context = {'contacts': contacts, 'users': users, 'contact_lists': contact_lists, 'con_tacts':con_tacts,
            'bounced_contacts': bounced_contacts, 'failed_contacts': failed_contacts, 'con_tacts':con_tacts
        }
        return render(request, 'search_contact_emails.html', context)
