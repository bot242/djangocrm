...
from common.models import User, Comment, Attachments, Currency
...
from opportunity.models import Opportunity, Updated_viewlog
...
def create_opportunity(request):
    ...
    if request.user.role != "ADMIN" and not request.user.is_superuser:
        accounts = Account.objects.filter(Q(
            created_by=request.user)|Q(assigned_to=request.user))
...
def update_opportunity(request, pk):
    ...
    obj = Opportunity.objects.filter(id=pk).values_list('stage', flat=True).first()
    print(obj, '-------------------------')
    obj_id = Opportunity.objects.filter(id=pk).values_list('id', flat=True).first()
    # print(obj_id,'----------------')
    date_id = Opportunity.objects.filter(id=pk).values_list('update_date', flat=True).first()
    print(date_id,"+++++++++++++++++++++++++++++++")

    # closed=Opportunity.objects.values_list('closed_on',flat=True).get(id=pk)
    # print('&&&',closed)
    accounts = Account.objects.filter(status="open")
    contacts = Contact.objects.all()
    if request.user.role != "ADMIN" and not request.user.is_superuser:
    ...
    if request.POST:
        ...
        if form.is_valid():
            assigned_to_ids = opportunity_object.assigned_to.all().values_list(
                'id', flat=True)
            opportunity_obj = form.save(commit=False)
            update_view = Updated_viewlog.objects.create(stage=obj, ud_id=obj_id, up_date=date_id)
            ...
            opportunity_obj.update_date=datetime.now()
            opportunity_obj.closed_on=p
            opportunity_obj.save()
...
def viewlog(request):
    ...
    update = Updated_viewlog.objects.all().filter(ud_id=request.session['pk']).order_by("id")
    print("updated result:",update)
    
    return render(request,'viewlog_opportunity.html', {'update':update})
