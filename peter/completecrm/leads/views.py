...
def lead_list_view(request):
    ...
    queryset = Lead.objects.all().exclude(status='Qualified'
    ...
    if  LeadNotification.objects.filter(userid=request.user.id).exists():
            if LeadNotification.objects.filter(userid=request.user.id,notifi_status='unread').exists():
                leadnoti=LeadNotification.objects.filter(userid=request.user.id,notifi_status='unread').count()
                print('***************&&&&&&&&&****************',leadnoti)
    ...
    if request.method == 'POST':
        ...
        search = True if (
            ...
            request.POST.get('assigned_to') or
        ...
        if request_post:
            ...
            if request_post.get('title'):
                queryset = queryset.filter(
                    city__icontains=request_post.get('title'))
...
class LeadListView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_queryset(self):
        queryset = self.model.objects.all().exclude(status='Qualified').select_related('created_by'
...
def convert_lead(request, pk):
    ...
    return HttpResponseRedirect(
        reverse('leads:edit_lead', kwargs={
            'pk': lead_obj.id}) + '?status=qualified')
